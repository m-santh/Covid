...
public class ActivityMain extends ActivityDrawer implements  FragmentScanner.OnScannerActionDelegate {
    ...
    public enum Feature {broadcast, scan, scanAdd, scanRestart}
        ...
        switch (feature) {
            ...
            case scanAdd:
                if(mViewPager.getCurrentItem() == ViewPagerAdapter.PAGE_INDEX_SCANNER) {
                    final FragmentScanner fragmentScanner = mViewPagerAdapter.getFragmentScanner();
                    fragmentScanner.actionAddEvent();
                }
                break;
            case scan:
                ...
                break;
            case scanRestart:
                BeaconScannerService.startScanning(getApplicationContext());
                break;
        }
    }
    ...
    private void buildScreen() {
        ...
        mViewPager.setCurrentItem(ViewPagerAdapter.PAGE_INDEX_SCANNER);
        String empNo = String.format("%8s", App.getInstance().getConfig().getEmpno()).replace(' ', '0');
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        buildScreen();
        App.getInstance().setActivityMain(this);
        String empNo = String.format("%8s", App.getInstance().getConfig().getEmpno()).replace(' ', '0');
        if((empNo.equals("00000000") )) {
            //: Force settings screen
            setContentView(R.layout.activity_frm);
            //Button mButton = (Button)findViewById(R.id.frm_button);
            EditText mButton = (EditText)findViewById(R.id.frm_empno);

            mButton.setOnClickListener(new View.OnClickListener() {
                public void onClick(View view) {
                    String regexStr = "^[0-9]*$";
                    EditText mEdit   = (EditText)findViewById(R.id.frm_empno);
                    String empNo = mEdit.getText().toString();


                    if ((empNo.length() != 0 ) && (empNo.trim().matches(regexStr) && (Integer.parseInt(empNo) > 0))) {
                        App.getInstance().getConfig().setEmpno(empNo);
                        //super.finish();
                        ...
                        mText.setTextColor(Color.parseColor("#B70808"));
    ...
    public FragmentScanner getFragmentScanner() {
        return mViewPagerAdapter.getFragmentScanner();
    }
