...
{
	public string DeviceName = "oneplus6";
	...
	public bool _onClickDisconnect;
	public bool _onClickWriteValue;
	public bool _onClickStopScanning;
	public bool _onClickConnect;

	public Text infoTxt;
	public Text labelTxt;

	public Toggle toggleRSSI;
	...
	public void InteractionManager()
	{
		if (_connected)
		{
			if (_state == States.None)
			{
				...
				if (_dataBytes != null)
				{
					...
					infoTxt.text = data.ToString();
				}
			}
			else if (_state == States.Subscribe && _timeout == 0f)
			{
				infoTxt.text = "Press the button on the RFduino";
			}
		}
		else if (_state == States.ScanRSSI)
		{
			...
			if (_rssi != 0)
			{
				labelTxt.text = "RSSI: {0} " + _rssi.ToString();
			}
		}
		else if (_state == States.None)
		{
			onClickConnect();
			_myRSSI = toggleRSSI.isOn;
		}
	}

	public void onClickDisconnect()
	{
		_onClickDisconnect = true;
		if (_onClickDisconnect == true)
		{
			SetState(States.Unsubscribe, 1f);
			_onClickDisconnect = false;
			Debug.Log("Came inside onClickDisconnect()");
		}
	}

	public void onClickWriteValue()
	{
		_onClickWriteValue = true;
		if (_onClickWriteValue == true)
		{
			OnLED();
			_onClickWriteValue = false;
			Debug.Log("Came inside onClickWriteValue()");
		}
	}

	public void onClickStopScanning()
	{
		_onClickStopScanning = true;
		if (_onClickStopScanning == true)
		{
			BluetoothLEHardwareInterface.StopScan();
			SetState(States.Disconnect, 0.5f);
			_onClickStopScanning = false;
			Debug.Log("Came inside onClickStopScanning()");
		}
	}

	public void onClickConnect()
	{
		_onClickConnect = true;
		if (_onClickConnect == true)
		{
			StartProcess();
			_onClickConnect = false;
			Debug.Log("Came inside onClickConnect()");
		}
	}
