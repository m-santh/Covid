...
public class iBeaconScannerAllDevice : MonoBehaviour
{
		...
		if (_timeout > 0f)
		{
			...
			if (_timeout <= 0f)
			{
				if (_startScan)
				{
					...
					//BluetoothLEHardwareInterface.ScanForPeripheralsWithServices(new string[] {""},(ibeaconData)=>{
					//BluetoothLEHardwareInterface.ScanForPeripheralsWithServices(string[] {""}, (""), Action<string, string, int, byte[]> actionAdvertisingInfo = null, bool rssiOnly = false, bool clearPeripheralList = true, int recordType = 0xFF)
